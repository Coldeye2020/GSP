...
class GSP(nn.Module):
    ...
    def train(self, loaders, test_set, metric_dict, use_edge_attr):
        ...
        for epoch in range(self.epochs):
            ...
            main_metric_idx = 1 # clf_roc or clf_acc
    ...
    def log_epoch(self, epoch, phase, loss_dict, att, clf_labels, clf_logits, batch, fine_tuning=False):
        ...
        eval_desc, clf_acc, clf_roc = self.get_eval_score(epoch, phase, att, clf_labels, clf_logits, batch, fine_tuning=fine_tuning)
        desc += eval_desc
        return desc, clf_acc, clf_roc, loss_dict['pred']

    def get_eval_score(self, epoch, phase, att, clf_labels, clf_logits, batch, fine_tuning):
        ...
        if fine_tuning:
            # ACC metric
            pred = (clf_logits > 0).float().squeeze(-1)
            clf_labels = clf_labels.squeeze(-1)
            clf_acc = (pred == clf_labels).sum().item() / clf_labels.shape[0]
        else:
    ...
    def few_shot_tuning(self, data_config, finetune_config, loaders, test_set, metric_dict, use_edge_attr, ood_flag):
        ...
        for epoch in range(self.epochs):
            ...
            main_metric_idx = 0 # clf_roc or clf_acc
...
def pretrain_gsp_one_seed(gsp, loaders_info, local_config, data_dir, model_name, dataset_name, method_name, device, random_state):
    ...
    loaders_sst2, loaders_sst5, test_set, x_dim, edge_attr_dim, num_tasks, aux_info = loaders_info
    
    print('====================================')
    print('[INFO] Pretraining GSP...')
    metric_dict = deepcopy(init_metric_dict)
    metric_dict = gsp.train(loaders_sst5, test_set, metric_dict, model_config.get('use_edge_attr', True))
...
def main():
    ...
    if dataset_name == 'Graph-SST':
        local_config["data_config"]["pretrain_task_idx"] = [0, 1, 2, 3, 4]
        local_config["data_config"]["finetune_task_idx"] = [5]
        local_config["data_config"]["batch_size"] = 128
        local_config["data_config"]["num_fs_samples"] = args.shot
        local_config["data_config"]["fs_batch_size"] = args.shot
        local_config['finetune_config']["epochs"] = 40
    ...
    for random_state in range(num_seeds):
        pretrain_model_dir = pretrain_dir / dataset_name / model_name / str(args.ood_flag)
        ...
        try:
            ...
            finetune_init_dict["loaders"] = loaders_sst2
